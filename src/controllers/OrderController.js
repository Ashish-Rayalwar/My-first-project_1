...
const updateController = async function(req,res){
    ...
    if(header===true){
        ...
        let updateStatus = await OrderDocument.find().select({_id:0,isFreeAppUser:1}).updateMany({isFreeAppUser:{$ne:false}},{$set:{isFreeAppUser:true}})
        ...
        let orderAmountUpdate = await OrderDocument.find().select({_id:0,amount:1}).updateMany({amount:{$gt:0}},{$set:{amount:productPrice[0]}})
        ...
        return res.send({resultBal:resultBalance,OrderAmount:orderAmountUpdate,status:updateStatus})
    }
